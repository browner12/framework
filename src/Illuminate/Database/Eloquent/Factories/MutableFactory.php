...
abstract class MutableFactory
{
    ...
    protected function getRawAttributes(?Model $parent)
    {
        return $this->states->pipe(function ($states) {
            return $this->for->isEmpty() ? $states : new Collection(array_merge([fn() => $this->parentResolvers()], $states->all()));
    ...
    {
        return $this->for->map(fn(BelongsToRelationship $for) => $for->attributesFor($this->newModel()))->collapse()->all();
    }
    ...
    {
        $this->states = $this->states->concat([is_callable($state) ? $state : fn() => $state]);
    ...
    {
        $this->has = $this->has->concat([new BelongsToManyRelationship(
            $factory,
            $pivot,
    ...
    {
        $this->for = $this->for->concat([new BelongsToRelationship(
            $factory,
